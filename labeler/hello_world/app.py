...
def write_s3(df, in_bucket, out_bucket, image_names):
	...
	response = s3_resource.Object(out_bucket, f'labels/{filename}').put(Body=csv_buffer.getvalue())
	copy_processed(out_bucket, image_names)
	delete_unprocessed(in_bucket, image_names)
	LOG.info(f'Result of write to "processed" folder in bucket: {out_bucket} with:\n {response}')	
...
def lambda_handler(event, context):
	...
	# Write result to S3
	write_s3(df=df, in_bucket=BUCKET, out_bucket=LABEL_BUCKET, image_names=image_names) #test lambda